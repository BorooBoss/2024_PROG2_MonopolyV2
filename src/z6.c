...
int roll_dice() { //hod kockou bo tamten nesiel
    return (rand() % 6) + 1;
}
...
void winner_p1(){
    ...
    printw("\n");
    printw("\n");
    printw("    W   W   III   N   N  N   N   EEEEE  RRRR        III    SSSS    \n");
    printw("    W   W    I    NN  N  NN  N   E      R   R        I    S          \n");
    printw("    W W W    I    N N N  N N N   EEEE   RRRR         I     SSS      \n");
    printw("    WW WW    I    N  NN  N  NN   E      R  R         I        S     \n");
    printw("    W   W   III   N   N  N   N   EEEEE  R   R      III   SSSSS      \n");
    printw("\n\n\n\n");
    printw("\n");
    printw("\n");
    printw("\n");
...
int main(void) {
    ...
    printw("\n\n\n\n");
    mvprintw(23,0,"Note: If the game resolution does not match yours, unzoom console and restart the program (Ctrl + C) on Linux\n");
    mvprintw(24,0,"GAME MODES:\n");
    mvprintw(25,0,"Press 1 for GAME MODE *TILL BANKRUPT* \n");
    mvprintw(26,0,"Press 2 for GAME MODE *TURNS NUMBER* (Player with biggest amount of cash wins)\n");
    int numTurns = 0;
    int menu_gameMode = 0;
    while (menu_gameMode != 1 || menu_gameMode != 2){ //kym nevyberes gamemode
    menu_gameMode = getch() - '0';
    ...
    mvprintw(31,0,"Press 7 for PLAYER VS PLAYER VS COMPUTER\n");
    int menu_players = 0;
    int num_players = 0;
    int bots = 0;
    while (menu_players != 1 || menu_players != 2 || menu_players != 3 || menu_players != 4 || menu_players != 5 || menu_players != 6 || menu_players != 7 ){
        menu_players = getch() - '0';
        if (menu_players == 1) { //PLAYER VS PLAYER
            ...
            bots = 3;
            break;
        } else if (menu_players == 7) { //PLAYER VS PLAYER VS COMPUTER
            num_players = 3;
            bots = 1;
            break;
        } else {
    ...
    clear();
    refresh();
    PLAYER *players = init(num_players); //zrob polia hracov
    int turns = 1;
    ...
    mvprintw(0, 0, "'Press 'Enter' after you wrote your number from dice. If you want to exit game, press 'Enter' without any argument. Sleep() is set on 1 second");
    refresh();

    //nakreslenie hracieho planu monopolov - pre 24 policok
    drawGameBoard(win, game_board, maxy, maxx);
    //umiestnenie hraca na START
    switch (num_players) { //vykresli hracov na start
        case 2:
    ...
    move(1, 0); //chcel som posunut ten blikajuci kurzor v strede ale neslo, hodnotiaci zmiluj sa :(
    printw(" ");
    refresh();
    while (game_ended != true) {
        ...
        for (int i = 0; i < num_players; i++) { //kazdy hrac hodi kockou
            ...
            if (bot_pocitadlo < bots){ //boti hadzu automaticky, hraci zadavaju cez scanf
                dice = roll_dice();
                bot_pocitadlo++;
            }
            else{
                ...
                player_locations[i] = new_location; // pocty na kreslenie novych a mazanie starych panacikov
                before_turn(players, &players[i], dice, &game_ended, num_players);
                if (new_location > 24){ //ak sme presli cele policko
                    new_location = new_location-24;
                }
                switch (i) { //kreslenie kazdeho hraca, mazanie starej polohy
                    case 0:
                    ...
                    if (i == 0){
                        if (new_location == 19) { //ak skocime na policko go to jail
                            if (players[0].num_jail_pass > 0) { //ak mame priespustky odratame jednu
                                ...
                                players[0].space_number -= 12; //dam do do jailu
                                players[0].is_in_jail = 1; //nastavim mu ze je vo vazeni
                                drawPlayer0(win, player_locations[0], player_locations[0]-12); //nakreslim ho v jaile
                                player_locations[0] -= 12;//nastavim vsetko jak ma byt - robim pre kazdu draw funkciu
                                new_location -= 12;
                                refresh();
            ...
            //WINDOW *win2 = newwin(11, 21, 18, 40); //VYKRESLENIE KOCKY V STREDE
            WINDOW *win2 = newwin(11, 25, 30, 67);
            wbkgd(win2, COLOR_PAIR(6));
            wrefresh(win2);
            wattron(win2, COLOR_PAIR(0));
            ...
            mvwprintw(win2, y_center, x_center, "%d", dice);
            ...
            mvprintw(52,150,"Last Turn: P%d\n", i + 1);
            //mvprintw(26,100,"\n");
            refresh();
            turns++;
            dice = 0;
            if (player_locations[i] > 24){ //skurvena picovina fakt ked som si pri dacom vytrhal vsetky vlasy tak pri tomto
                 player_locations[i] = new_location;
            }
            if (menu_gameMode == 2){ //ked to je mod 2, tak odpocitavaj koleck, urci vitaza podla penazi
                numTurns = numTurns -1;
                if (numTurns == 0){
                    //game_ended = true;
                    if (num_players == 4){
                        if ((players[0].cash > players[1].cash && players[0].cash > players[2].cash) && players[0].cash > players[3].cash){
                            clear();
                            winner_p1();
                            refresh();
                        }
                        else if ((players[2].cash > players[0].cash && players[1].cash < players[2].cash)&& players[2].cash > players[3].cash){
                            clear();
                            winner_p3();
                            refresh();
                        }
                        else if ((players[0].cash < players[1].cash && players[2].cash < players[1].cash) && players[1].cash > players[3].cash){
                            clear();
                            winner_p2();
                            refresh();
                        }
                        else if ((players[0].cash < players[3].cash && players[2].cash < players[3].cash) && players[3].cash > players[1].cash){
                            clear();
                            winner_p3;
                            refresh();
                        }
                        ...
                        if (players[0].cash > players[1].cash && players[0].cash > players[2].cash){
                            clear();
                            winner_p1();
                            refresh();
                        }
                        else if (players[2].cash > players[0].cash && players[1].cash < players[2].cash){
                            clear();
                            winner_p3();
                            refresh();
                        }
                        else if (players[0].cash < players[1].cash && players[2].cash < players[1].cash){
                            clear();
                            winner_p2();
                            refresh();
                        }
                        ...
                        if (players[0].cash > players[1].cash){
                            clear();
                            winner_p1();
                            refresh();
                        }
                        else if (players[1].cash > players[0].cash){
                            clear();
                            winner_p1();
                            refresh();
                        }
